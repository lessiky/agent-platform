...
type AgentService interface {
	...
	// GetInvokeExecution 查询 /invoke 执行任务状态 (API Key 鉴权; 仅限本 Agent 的执行任务)
	GetInvokeExecution(ctx context.Context, agentID, plainKey, executionID string) (*model.AgentExecution, error)
	// CancelInvokeExecution 取消 /invoke 执行任务 (API Key 鉴权): 外部方主动放弃进行中的任务,
	// 平台取消执行上下文 (透传进行中的模型/MCP 调用) 并标记终态 cancelled; 已终态任务幂等返回
	CancelInvokeExecution(ctx context.Context, agentID, plainKey, executionID string) (*model.AgentExecution, bool, error)
...
// InvokeAgentResult API Key 外部调用结果:
// 异步路径 (2026-08-24) 返回 execution_id, 状态/结果经执行任务端点查询;
// 降级路径 (无可用模型) 保持同步旧链响应 (reply 为空, model_ok=false)
type InvokeAgentResult struct {
	AgentID          string                    `json:"agent_id"`
	KeyPrefix        string                    `json:"key_prefix"`
	Status           string                    `json:"status,omitempty"`       // 执行任务状态 (异步路径: running)
	ExecutionID      string                    `json:"execution_id,omitempty"` // 执行任务 ID (异步路径, 202)
	Reply            string                    `json:"reply"`                  // 模型应答 (降级路径)
	Model            string                    `json:"model,omitempty"`
...
func (s *agentService) InvokeAgent(ctx context.Context, agentID, plainKey string, req InvokeAgentRequest) (*InvokeAgentResult, error) {
	...
	// 执行调用链 (2026-08-24 升级为异步执行任务):
	// - 无可用模型 → 降级旧链路 (MCP 调用 + token 估算), 保持 model_ok=false 同步语义
	// - 模型可用 → 异步执行任务: HTTP 立即返回 202 + execution_id,
	//   状态/阶段/结果经 GET /agents/:id/invoke/executions/:executionId 查询
	if !s.modelSvc.ModelAvailable(ctx, agentID) {
		return s.invokeLegacy(ctx, agentID, key, req.Message), nil
	}
	execution, err := s.chat.InvokeAsync(ctx, agentID, InvokeRequest{Message: req.Message, SessionID: req.SessionID})
	if err != nil {
		return nil, err
	}
...
// GetInvokeExecution 查询 /invoke 执行任务状态 (API Key 鉴权):
// /invoke 返回 202 (execution_id) 后, 外部系统凭 API Key 轮询本视图:
// status 进入 success/failed/stalled 即终态; success 时 result 为模型应答与调用明细
func (s *agentService) GetInvokeExecution(ctx context.Context, agentID, plainKey, executionID string) (*model.AgentExecution, error) {
	if _, err := s.verifyInvokeKey(ctx, agentID, plainKey); err != nil {
		return nil, err
	}
	if _, err := uuid.Parse(executionID); err != nil {
		return nil, errors.NewValidationError("invalid execution_id")
	}
	return s.chat.GetExecution(ctx, agentID, executionID)
}

// CancelInvokeExecution 取消 /invoke 执行任务 (API Key 鉴权, 仅限本 Agent 的执行任务):
// 外部方主动放弃进行中的任务 → 取消执行上下文 (透传进行中的模型/MCP 调用) 并标记终态 cancelled;
// 已终态 (success/failed/stalled/cancelled) 幂等返回当前状态; waiting_approval / 不在本进程 → 409
func (s *agentService) CancelInvokeExecution(ctx context.Context, agentID, plainKey, executionID string) (*model.AgentExecution, bool, error) {
	if _, err := s.verifyInvokeKey(ctx, agentID, plainKey); err != nil {
		return nil, false, err
	}
	if _, err := uuid.Parse(executionID); err != nil {
		return nil, false, errors.NewValidationError("invalid execution_id")
	}
	return s.chat.CancelInvokeExecution(ctx, agentID, executionID)
}
