...
func TestRunToolRoundsHaltOnApproval(t *testing.T) {
	...
	if _, err := s.runToolRounds(context.Background(), "agent-1", &messages, outcome, nil, toolIndex, modelclient.GenOptions{}, 5, "sess-1", "chat", model.ApprovalSourceChat, &pending, &calls, "exec-1", nil, nil, nil, false, nil); err != nil {
