...
func buildAIGenerateSystemPrompt(catalog *aiGenCatalog) string {
	...
	b.WriteString("- type=print: 打印输出 (执行历史「工作流输出」按节点文字颜色展示, 适合流程播报/留痕)。config: {message(输出内容, 支持变量), color?(文字颜色 hex 如 #1677ff, 省略为默认色)}。输出: message、color\n")
	b.WriteString("- agent 节点的 agent_id 与 mcp_tool 节点的 mcp_server_id/tool 必须来自下方目录, 不得编造; 若所需资源不存在, 用 http/delay/condition/print 节点替代, 并在 description 中说明\n\n")
